...
# Model setup
num_labels = 5
model = Classifier(AutoModel.from_pretrained(checkpoint, config=AutoConfig.from_pretrained(checkpoint)), num_labels).to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))

# Dataloader
data_collator = DataCollatorWithPadding(tokenizer=tokenizer)
train_dataloader = DataLoader(tokenized_dataset["train"], shuffle=True, batch_size=16, collate_fn=data_collator)
eval_dataloader = DataLoader(tokenized_dataset["test"], batch_size=16, collate_fn=data_collator)
...
for lr, batch_size, num_epochs in itertools.product(learning_rates, batch_sizes, num_epochs_options):
    ...
    train_dataloader = DataLoader(tokenized_dataset["train"], shuffle=True, batch_size=batch_size, collate_fn=DataCollatorWithPadding(tokenizer))
    eval_dataloader = DataLoader(tokenized_dataset["test"], batch_size=batch_size, collate_fn=DataCollatorWithPadding(tokenizer))

    # Initialize model for current hyperparameters
    bert_model = AutoModel.from_pretrained(checkpoint, config=AutoConfig.from_pretrained(checkpoint))
    model = Classifier(bert_model, num_labels).to(device)
    optimizer = AdamW(model.parameters(), lr=lr)
    lr_scheduler = get_scheduler("linear", optimizer=optimizer, num_warmup_steps=0, num_training_steps=num_epochs * len(train_dataloader))

    # Train and evaluate the model
    accuracy = train_and_evaluate(model, train_dataloader, eval_dataloader, optimizer, lr_scheduler, device, num_epochs)
